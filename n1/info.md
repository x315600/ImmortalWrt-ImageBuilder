...
### 斐讯N1为单网口旁路由模式：默认IP `192.168.2.254`，主路由/网关/DNS 为 `192.168.2.1`，已关闭N1自身DHCP<br> 已启用LAN侧IPv6客户端（RA/DHCPv6），并将 `br-lan` + `docker0` 直接加入 `lan` 防火墙区域，容器与LAN双向直通<br> 接入同网段后访问 `http://192.168.2.254`
